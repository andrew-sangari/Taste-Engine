...
export async function POST(request: Request) {
  const user = await getChatGPTUser();
  if (!user) return Response.json({ error: "Sign in with ChatGPT to plan a night out." }, { status: 401 });
  ...
  const projection = await loadProjection({ id: profile.id, allowBundledFallback: profile.legacyDefault });
  if (!projection) {
