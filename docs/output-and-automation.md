...
Framework is a followed promoter calendar and supplies a low-frequency public artist roster; exact roster names expand deterministic SeatGeek performer and Ticketmaster attraction lookups and contribute promoter evidence without overriding direct taste signals. The Insomniac adapter remains explicitly unavailable until verified against permitted fixtures. All sources are deduplicated into the same candidate contract, may fail independently, and remain visible in grouped source health and the separate provider filter. Overview candidates are explicitly queued for advisory passes ahead of general caps; deterministic ranking remains authoritative.
