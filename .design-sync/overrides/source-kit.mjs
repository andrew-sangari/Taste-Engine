// forked from design-sync lib/source-kit.mjs — site/app is a Next.js
// app-router tree; reserved route filenames (layout/page/loading/error/...)
// execute Next-only code (next/font, next/headers, "./globals.css" ->
// @import "tailwindcss") at module top-level, which breaks a synth-entry
// `export *` bundle even when componentSrcMap excludes their component
// names, because entry synthesis re-exports every src file regardless of
// that map. NON_IMPL_RX below adds those filenames so they're excluded from
// synth-entry file selection AND from deriveComponentsFromSrc scanning.
...
const NON_IMPL_RX = /\.(stories|test|spec)\.|(?:^|\/)(?:layout|page|loading|error|not-found|template|default|route|global-error)\.(tsx|jsx|ts|js)$/;
