...
- **The durable sync state was only committed on the unmerged `codex/history-driven-feedback` branch** (commit 1807200), so a master checkout lost `.design-sync/config.json` and friends. This run restored them via `git checkout 1807200 -- .design-sync` and re-added the design-sync `.gitignore` entries to master's `.gitignore`. Commit the durable set on master so this doesn't recur.
- `cssEntry` hash rotated as predicted (`index-BTcwYKcL.css` → `index-DwLmCcjp.css`) after the v2 visual design site rebuild — updated in config. Expect this on every site rebuild.
- v2 visual design added 3 exports: `HassleDial` and `UrgencyChip` (authored previews, graded good) and `Reveal` — excluded via `componentSrcMap` (returns `null`; it's an IntersectionObserver scroll-reveal/count-up behavior component with no visual surface, same category as `FeedbackProvider`).
- Token palette and class vocabulary survived the v2 redesign unchanged — `conventions.md` validated against the fresh build with zero drift, no edits needed.
- `UrgencyChip` real app values are `"watch"` and `"safe to wait"` (only `watch` gets the accent pulse); `HassleDial`'s ticks flip to warm orange at `score >= 6`.
