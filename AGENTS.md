...
Production is migrating to Sites + D1. The site reads active recommendation snapshots from D1 with the accepted bundled projection as a recovery fallback, stores signed-in feedback in D1, and owns hosted Spotify PKCE/token refresh, playlist reads, selection, and Top Artists cache. Ollama Cloud is the target optional advisory runtime. See `docs/hosted-runtime-migration.md`.
...
- `npm run nightlife:shadow` — compare deterministic and inference-assisted nightlife shortlists side by side; changes no ranking or publication state
...
- Taste Engine owns hosted read-only Spotify authentication and evidence. Playlist Sync remains the local parity and recovery adapter during cutover; production must not call localhost.
...
This layer launches in shadow/advisory mode. It may order its own nightlife surface, but it must not change the canonical utility score, source facts, publication eligibility, or the learned taste profile. Schedule, travel, overlap, budget, and window feasibility are always recomputed deterministically. See `docs/system-one-inference.md`.
...
- Hosted production must continue while the desktop and computer are off. During cutover, recovery refreshes still require the project at its configured path. Ollama Cloud is optional; otherwise deterministic copy publishes.
